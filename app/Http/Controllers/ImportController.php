...
class ImportController extends Controller implements HasMiddleware
{
    ...
    {
        return [
            new Middleware('permission:view import', only: ['index']),
            new Middleware('permission:edit import', only: ['edit','update']),
            new Middleware('permission:create import', only: ['create','store']),
            new Middleware('permission:delete import', only: ['destroy']),
        ];
    }
    // نمایش لیست واردات
