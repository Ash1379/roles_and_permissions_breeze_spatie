...
class ChequeController extends Controller implements HasMiddleware
{
    ...
    {
        return [
            new Middleware('permission:view cheque', only: ['index']),
            new Middleware('permission:edit cheque', only: ['edit','update']),
            new Middleware('permission:create cheque', only: ['create','store']),
            new Middleware('permission:delete cheque', only: ['destroy']),
        ];
    }
    // ✔ نمایش لیست چک‌ها
    public function index()
    {
        $cheques = Cheque::latest()->paginate(100);
