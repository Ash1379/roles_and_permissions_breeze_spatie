...
<div class="content-page">
    <div class="content">
        ...
        <div class="container-xxl py-3 d-flex justify-content-between align-items-center">
            <h2 class="fw-semibold fs-4 text-dark m-0">Edit Payment</h2>
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <form action="{{ route('payments.update', $payment->id) }}" method="POST">
                                ...
                                <div class="mb-3">
                                    <label>Amount</label>
                                    <input type="number" name="amount" step="0.01" value="{{ old('amount', $payment->amount) }}" class="form-control" required>
                                    @error('amount')<p class="text-danger mt-1">{{ $message }}</p>@enderror
                                </div>

                                <div class="mb-3">
                                    <label>Method</label>
                                    <input type="text" name="method" value="{{ old('method', $payment->method) }}" class="form-control" required>
                                    @error('method')<p class="text-danger mt-1">{{ $message }}</p>@enderror
                                </div>

                                <div class="mb-3">
                                    <label>Date</label>
                                    <input type="date" name="date" value="{{ old('date', $payment->date->format('Y-m-d')) }}" class="form-control" required>
