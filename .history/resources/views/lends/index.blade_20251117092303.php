...
@section('content')
<div class="container">
    <h1 class="mb-4">Lends</h1>
    <a href="{{ route('lends.create') }}" class="btn btn-primary mb-3">Add Lend</a>
        ...
        <tbody>
            @foreach($lends as $lend)
                <tr>
                    <td>{{ $lend->id }}</td>
                    <td>{{ $lend->customer->name }}</td>
                    <td>{{ number_format($lend->amount, 2) }}</td>
                    <td>{{ \Carbon\Carbon::parse($lend->date)->format('d-m-Y') }}</td>
                    <td>{{ $lend->description }}</td>
                    <td>
                        <a href="{{ route('lends.edit', $lend->id) }}" class="btn btn-sm btn-warning">Edit</a>
                    </td>
