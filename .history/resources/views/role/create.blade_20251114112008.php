            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div>
                            ...
                            <div class="grid grid-cols-4 md-3">
                                @if($permissions->isNotEmpty())
                                    @foreach($permissions as $permission)
                                        <div class="mt-3">
                                            <input type="checkbox" name="permissions[]" value="{{ $permission->name }}" class="border-gray-300 text-black shadow-sm rounded-lg">
                                            <label for="permissions[]" class="text-sm font-medium">{{ $permission->name }}</label>
                                        </div>
                                    @endforeach
