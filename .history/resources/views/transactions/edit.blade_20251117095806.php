                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('cheques.update', $cheque->id) }}" method="post">
                        ...
                        <div>
                            ...
                            <div class="my-3">
                                <input type="text" name="cheque_number" value="{{ old('cheque_number', $cheque->cheque_number) }}" class="border-gray-300 text-black shadow-sm w-1/2 rounded-lg">
                                @error('cheque_number')<p class="text-red-500">{{ $message }}</p>@enderror
                            </div>

                            <label class="text-sm font-medium">Amount</label>
                            <div class="my-3">
                                <input type="number" step="0.01" name="amount" value="{{ old('amount', $cheque->amount) }}" class="border-gray-300 text-black shadow-sm w-1/2 rounded-lg">
                                @error('amount')<p class="text-red-500">{{ $message }}</p>@enderror
                            </div>

                            <label class="text-sm font-medium">Bank</label>
                            <div class="my-3">
                                <input type="text" name="bank" value="{{ old('bank', $cheque->bank) }}" class="border-gray-300 text-black shadow-sm w-1/2 rounded-lg">
                                @error('bank')<p class="text-red-500">{{ $message }}</p>@enderror
                            </div>

                            <label class="text-sm font-medium">Due Date</label>
                            <div class="my-3">
                                <input type="date" name="due_date" value="{{ old('due_date', $cheque->due_date->format('Y-m-d')) }}" class="border-gray-300 text-black shadow-sm w-1/2 rounded-lg">
