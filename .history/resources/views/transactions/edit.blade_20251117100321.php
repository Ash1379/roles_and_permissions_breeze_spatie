                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('transactions.update', $transaction->id) }}" method="post">
                        ...
                        <div>
                            ...
                            <div class="my-3">
                                <input type="number" name="reference_id" value="{{ old('reference_id', $transaction->reference_id) }}" class="border-gray-300 text-black shadow-sm w-1/2 rounded-lg">
                                @error('reference_id')<p class="text-red-500">{{ $message }}</p>@enderror
                            </div>

                            <label class="text-sm font-medium">Amount</label>
                            <div class="my-3">
                                <input type="number" step="0.01" name="amount" value="{{ old('amount', $transaction->amount) }}" class="border-gray-300 text-black shadow-sm w-1/2 rounded-lg">
                                @error('amount')<p class="text-red-500">{{ $message }}</p>@enderror
                            </div>

                            <label class="text-sm font-medium">Transaction Date</label>
                            <div class="my-3">
                                <input type="date" name="transaction_date" value="{{ old('transaction_date', $transaction->transaction_date->format('Y-m-d')) }}" class="border-gray-300 text-black shadow-sm w-1/2 rounded-lg">
                                @error('transaction_date')<p class="text-red-500">{{ $message }}</p>@enderror
                            </div>

                            <label class="text-sm font-medium">Notes</label>
                            <div class="my-3">
                                <textarea name="notes" class="border-gray-300 text-black shadow-sm w-1/2 rounded-lg">{{ old('notes', $transaction->notes) }}</textarea>
                            </div>
