<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('products.create') }}" class="bg-slate-700 text-sm rounded-md text-white px-5 py-3 hover:bg-slate-600 transition">Create</a>
        </div>
    </x-slot>
            ...
            <div class="overflow-x-auto bg-white shadow rounded-lg">
                <table class="w-full table-auto">
                    ...
                    <tbody class="bg-gray-50">
                        ...
                        <tr id="product-row-{{ $p->id }}" class="border-b hover:bg-gray-50 transition">
                            <td class="px-6 py-4">{{ $p->id }}</td>
                            <td class="px-6 py-4">{{ $p->name }}</td>
                            <td class="px-6 py-4">{{ $p->unit }}</td>
                            <td class="px-6 py-4">{{ $p->price }}</td>
                            <td class="px-6 py-4 space-x-2">
                                <a href="{{ route('products.edit',$p->id) }}" class="bg-blue-600 text-sm rounded-md text-white px-5 py-2 hover:bg-blue-700 transition">Edit</a>
                                <button onclick="deleteProduct({{ $p->id }})" class="bg-red-500 text-sm rounded-md text-white px-5 py-2 hover:bg-red-400 transition">Delete</button>
                            </td>
                        </tr>
                        @empty
                        <tr>
