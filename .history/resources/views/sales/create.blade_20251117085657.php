                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('sales.store') }}" method="post">
                        ...
                        <div class="my-3">
                            ...
                            <input type="number" step="0.001" name="litres" value="{{ old('litres') }}" class="border-gray-300 text-black shadow-sm w-1/2 rounded-lg">
                            @error('litres')<p class="text-red-500 font-medium">{{ $message }}</p>@enderror
                        </div>

                        <div class="my-3">
                            <label class="text-sm font-medium">Rate</label>
                            <input type="number" step="0.01" name="rate" value="{{ old('rate') }}" class="border-gray-300 text-black shadow-sm w-1/2 rounded-lg">
                            @error('rate')<p class="text-red-500 font-medium">{{ $message }}</p>@enderror
                        </div>

                        <div class="my-3">
                            <label class="text-sm font-medium">Total</label>
                            <input type="number" step="0.01" name="total" value="{{ old('total') }}" class="border-gray-300 text-black shadow-sm w-1/2 rounded-lg">
                            @error('total')<p class="text-red-500 font-medium">{{ $message }}</p>@enderror
                        </div>

                        <div class="my-3">
                            <label class="text-sm font-medium">Sold At</label>
                            <input type="datetime-local" name="sold_at" value="{{ old('sold_at') }}" class="border-gray-300 text-black shadow-sm w-1/2 rounded-lg">
